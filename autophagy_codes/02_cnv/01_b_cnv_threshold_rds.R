...
process_raw_data <- function(.x){
  ...
  d <- 
    readr::read_tsv(file = file.path(path, .x, "all_thresholded.by_genes.txt"), progress = F) %>% 
...
pancan_cnv %>% readr::write_rds(path = file.path(tcga_path, 'pancan34_cnv_threshold.rds.gz'), compress = "gz")
