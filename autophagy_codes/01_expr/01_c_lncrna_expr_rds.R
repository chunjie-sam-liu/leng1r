...
files.names <- list.files("/extraspace/yye1/share_data/TCGA_lncRNA", pattern = "*.tsv")
...
process_raw_data <- function(.x){
  print(.x)
  path <- "/extraspace/yye1/share_data/TCGA_lncRNA"
...
cancers_names %>%
  ...
  dplyr::as_tibble() %>%
  dplyr::ungroup() %>%
  dplyr::select(-PARTITION_ID, -names) -> pancan_lncrna
on.exit(parallel::stopCluster(cluster))
pancan_lncrna %>% readr::write_rds(path = file.path(tcga_path, 'pancan20_lncrna_expr.rds.gz'), compress = "gz")
