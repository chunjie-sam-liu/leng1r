...
mutationDir <- file.path(dataRootPath, "03.somatic/04.targetGenesForMutation")
...
getAroundGenes <- function(x, len){
  ...
  around.genes<-
    getBM(attributes = c("ensembl_gene_id",
          ...
          filters = c("chromosome_name", 
                      ...
                      "end",
                      "biotype"),
          values = list(chromosome_name= str_replace(x["chrom"], "chr", ""), 
  ...
  strand.plus <-
    filter(around.genes,
           strand == 1 & start_position - as.numeric(x["pos"]) > 0) %>%
    slice(which.min(start_position - as.numeric(x["pos"]))) %>%
    mutate(distance = start_position - as.numeric(x["pos"]),
           mutation = x['mutation'], 
           feature = x['feature'],
           ensr = x['ensr'],
  ...
  strand.minus <-
    filter(around.genes,
           strand == -1 & end_position - as.numeric(x["pos"]) < 0) %>%
...
collectGenes <- read_tsv(file = file.path(refDir, "collectedOncoAndTumorsupressorGenes.tsv"))
