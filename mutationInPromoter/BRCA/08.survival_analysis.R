...
mutation_target <- 
  read_rds(path = file.path(targetGeneDir, "nearest_target_coding_genes_3.rds")) %>%
  tbl_df() %>%
  rename(ensid = ensg)
...
rnaseq <- 
  ...
  filter(!str_detect(type, "NA"), type == "tumor") %>%
  distinct() %>%
...
# Load clinical data
clinical <- 
  read_rds(path = file.path(survivalPath, "BRCA_clinical_clean.rds")) %>%
  rename(time = os_days) %>%
...
survival_survfit_model <-
  for_survival %>% 
  anti_join(survival_coxph_model %>% filter(p.value < 0.05), by = "ensid") %>% 
  group_by(ensid) %>%
...
for_survival %>% 
  anti_join(
    survival_coxph_model %>% filter(p.value < 0.05), by = "ensid"
  ) -> 
  for_survival_filter

plot_survival <- function (df){
  fit <- survfit(Surv(time,status) ~ group, data = df, na.action = na.exclude)
  name <- unique(df$symbol)
  ggsurvplot(
    fit,
    data = df,
    pval = T,
    pval.method = T,
    xlab = "Overall survival in days",
    ylab = "Probability of survival",
    title = paste(name, "survival")
  ) ->
    plot
  res <- 
    arrange_ggsurvplots(list(plot), ncol = 1, nrow = 1, print = FALSE)
  
}

for_survival_filter %>% 
  left_join(target_gene, by = "ensid") %>% 
  group_by(ensid) %>% 
  by_slice(
    plot_survival,
    .labels = T,
  ...
  transmute(
    filename = paste(ensid, "survival.png", sep = "."),
    plot = plot
  ) ->
  survplots
...
survplots %>% 
  pwalk(
    ggsave,
    path = file.path(survivalPath, "ensg_survival"),
    device = "png",
    width = 7,
    height = 7
  )
